...
contract UploadSnapshotToStoreScript {
    function run() external {
        ...
        bytes memory snapshotBytes = vm.readFileBinary(vm.envString("PUREVM_SNAPSHOT_FILE"));

        vm.startBroadcast();
        store.uploadSnapshot(taskId, ordinal, snapshotBytes);
        vm.stopBroadcast();
    }
}
